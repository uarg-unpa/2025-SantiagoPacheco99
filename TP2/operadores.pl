...
es_par(Par):- Par mod 2 =:= 0.  % =:= es el igual

stock(el_principito,10).
libro_disponible_caro(Titulo, Precio, Stock):-stock(Titulo,Stock), Precio > 50. % Se aplica el operador &

clasificar_libro(Precio, Categoria) :- Precio > 50, Categoria = caro. % Se implementa el operador or
clasificar_libro(Precio, Categoria) :- Precio =< 50, Categoria = economico.


% Negacion por fallo
% Es verdadero si prolog no lo puede probar
sin_stock(Titulo) :- \+ stock(Titulo,_). % El _ es una variable anonima, que funciona como comodin, sirve para buscar el Titulo con cualquier cosa
...
% Lo que se declara dinamico o dynamic se hace al principio del archivo
...
disponible(Titulo) :- libro(Titulo, _), \+ prestado(Titulo, _). % Acordarse: El _ es un comodin
...
convertir_horas :-
    ...
    read(Horas),
number(Horas),  % Verifica que Horas sea un número, si da falso hace el backtracking y va al otro convertir_horas (el del error) para volver a ingresar por teclado
    Minutos is Horas * 60,
